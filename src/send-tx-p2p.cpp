...
{
    log_info() << boost::format(SX_SEND_TX_P2P_SIGNAL) % signal 
        /*<< std::endl*/;
    stopped = true;
}
...
static void handle_start(const std::error_code& error)
{
    if (error)
    {
        log_warning() << boost::format(SX_SEND_TX_P2P_START_FAIL) % 
    ...
    log_debug() << SX_SEND_TX_P2P_START_OKAY;
}
    ...
    size_t connection_count, size_t node_count)
{
    if (error)
    {
        log_warning() << boost::format(SX_SEND_TX_P2P_CHECK_FAIL) %
            error.message() /*<< std::endl*/;
        result = console_result::failure;
        stopped = true;
        return;
    ...
    log_debug() << boost::format(SX_SEND_TX_P2P_CHECK_OKAY) %
    ...
    protocol& prot, transaction_type& tx)
{
    if (error)
    {
        log_warning() << boost::format(SX_SEND_TX_P2P_SETUP_FAIL) %
            error.message() /*<< std::endl*/;
        result = console_result::failure;
        stopped = true;
        return;
    ...
    std::cout << boost::format(SX_SEND_TX_P2P_SETUP_OKAY) %
    ...
    auto handle_send = [](const std::error_code& error)
    {
        if (error)
        {
            log_warning() << boost::format(SX_SEND_TX_P2P_SEND_FAIL) %
                error.message() /*<< std::endl*/;
            result = console_result::failure;
            return;
        }

        std::cout << boost::format(SX_SEND_TX_P2P_SEND_OKAY) % now()
            << std::endl;
    };

    node->send(tx, handle_send);
    prot.subscribe_channel(std::bind(tx_send, ph::_1, ph::_2, std::ref(prot),
        std::ref(tx)));
}
...
console_result send_tx_p2p::invoke(std::ostream& output, std::ostream& cerr)
{
    ...
    prot.subscribe_channel(
        std::bind(tx_send, ph::_1, ph::_2, std::ref(prot), tx));
