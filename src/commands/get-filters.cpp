...
console_result get_filters::invoke(std::ostream& output,
    std::ostream& error)
{
    // Bound parameters.
    ...
    message::get_compact_filters request(neutrino_filter_type,
        ...
        send_handler](const code& ec, network::channel::ptr node)
    {
        ...
        node->subscribe<message::compact_filter>(std::move(receive_handler));
