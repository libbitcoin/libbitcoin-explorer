...
#define SX_SEED_BITS_TO_TERMINATED_BASE16(bits) (2 * (bits / 8) + 1)

BOOST_AUTO_TEST_CASE(seed__invoke__minimum_size__okay_output)
{
    SX_DECLARE_COMMAND(seed);
    ...
    BOOST_REQUIRE(output.str().size() == SX_SEED_BITS_TO_TERMINATED_BASE16(128));
}
...
BOOST_AUTO_TEST_CASE(seed__invoke__good_size__okay_output)
{
    ...
    SX_REQUIRE_OKAY(command.invoke(output, error));
    BOOST_REQUIRE(output.str().size() == SX_SEED_BITS_TO_TERMINATED_BASE16(256));
}
...
BOOST_AUTO_TEST_CASE(seed__invoke_big_size_okay_output)
{
    ...
    SX_REQUIRE_OKAY(command.invoke(output, error));
    BOOST_REQUIRE(output.str().size() == SX_SEED_BITS_TO_TERMINATED_BASE16(512));
}
...
BOOST_AUTO_TEST_CASE(seed__invoke__too_short__failure_error)
{
    ...
    SX_REQUIRE_ERROR(SX_SEED_BIT_LENGTH_UNSUPPORTED "\n");
}
