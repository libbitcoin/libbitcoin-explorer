...
BOOST_AUTO_TEST_CASE(hd_new__invoke__vector_1_master__okay_output)
{
    ...
    command.set_entropy_argument({ "000102030405060708090a0b0c0d0e0f" });
...
BOOST_AUTO_TEST_CASE(hd_new__invoke__vector_2_master__okay_output)
{
    BX_DECLARE_COMMAND(hd_new);
    command.set_version_option(76066276);
    command.set_entropy_argument({ "fffcf9f6f3f0edeae7e4e1dedbd8d5d2cfccc9c6c3c0bdbab7b4b1aeaba8a5a29f9c999693908d8a8784817e7b7875726f6c696663605d5a5754514e4b484542" });
    BX_REQUIRE_OKAY(command.invoke(output, error));
...
BOOST_AUTO_TEST_CASE(hd_new__invoke__64_bit_seed_mainnet__failure_error)
{
    ...
    BX_REQUIRE_ERROR(BX_HD_NEW_SHORT_ENTROPY "\n");
}
