...
build_all()
{
    ...
    build_from_github pmienk libbitcoin-client logging $PARALLEL ${BITCOIN_CLIENT_OPTIONS[@]} "$@"
    build_from_github pmienk libbitcoin-network logging $PARALLEL ${BITCOIN_NETWORK_OPTIONS[@]} "$@"
    build_from_travis pmienk libbitcoin-explorer logging $PARALLEL ${BITCOIN_EXPLORER_OPTIONS[@]} "$@"
}
